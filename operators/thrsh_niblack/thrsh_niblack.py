...
'''
@file thrsh_niblack.py
@author Scott L. Williams.
@package POLI
@brief skimage niblack thresholding
@LICENSE
# 
...
'''
thrsh_niblack_copyright = 'thrsh_niblack.py Copyright (c) 2010-2025 Scott L. Williams released under GNU GPL V3.0'
...
# if not then assume non-graphics implementaion
    ...
    from op import op
    operator = op
    eprint( 'thrsh_niblack: using non-graphics mode.' )

def get_name():
    return 'thrsh_niblack'

# return an instance of 'thresh' class 
def instantiate():	
    return thrsh_niblack( get_name() )

class thrsh_niblack_parameters( pio ):
    ...
    def print_params( self ):
        ...
        eprint( '\nparameters used for thrsh_niblack:' )
...
class thrsh_niblack( operator ):
    ...
    def __init__( self, name ): # initialize op_panel but no graphics
        ...
        self.p = thrsh_niblack_parameters()
    ...
    def run( self ):                     # override superclass run
        ...
        t_niblack = skimage.filters.threshold_niblack( self.source,
    ...
    def read_params_from_panel( self ):
        ...
        if wsize < 0:
            eprint( 'thrsh_niblack: read_params_from_panel:' )
    ...
    def first_panel( self ):
        ...
        self.t_k = wx.TextCtrl( p_fpanel, -1,
                                    style=wx.ALIGN_RIGHT, size=(30,15) )
        self.t_k.SetToolTip( 'Value of parameter k in threshold formula.' )
    ...
    def set_params( self, argv ):
        ...
        for opt, arg in opts:
            ...
            if opt in ( '-h', '--help' ):      
                ...
                wsize =  int( arg )
                if wsize < 1:
                    eprint( 'thrsh_niblack: wsize cannot be < 1...exiting' )
                    sys.exit( 2 )
                self.p.wsize = wsize
                ...
                if not os.path.isfile( arg ):
                    eprint( 'thrsh_niblack: set_params:' )
        ...
        if params != None:
            ...
            if not ok:
                eprint( 'thrsh_niblack: cannot read parm file:', params,
                        ' ...exiting' )
                sys.exit( 2 )
