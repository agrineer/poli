...
@package POLI
@brief skimage triangle thresholding
@LICENSE
#
#  thrsh_tri.py Copyright (C) 2010-2025 Scott L. Williams.
# 
...
Threshold operator using the triangle algorithm from skimage
Returns:
...
thrsh_tri_copyright = 'thrsh_tri.py Copyright (c) 2010-2025 Scott L. Williams, released under GNU GPL V3.0'
...
# if not then assume non-graphics implementaion
    ...
    from op import op
    operator = op
    eprint( 'thrsh_tri: using non-graphics mode.' )
...
def instantiate():	
    return thrsh_tri( get_name() )

class thrsh_tri_parameters( pio ):
    ...
    def print_params( self ):
        ...
        eprint( '\nparameters for thrsh_tri:' )
        eprint( '      binary =', self.binary  )
        
# ----------------------------------------------------------------------------

class thrsh_tri( operator ):
    ...
    def run( self ):                    # override superclass run
        ...
        t_tri = skimage.filters.threshold_triangle( self.source )
        
        mask = self.source > t_tri      # boolean mask
    ...
    def set_params( self, argv ):
        ...
        if params != None:
            ...
            if not ok:
                eprint( 'thrsh_tri: cannot read parm file:', params,
                        ' ...exiting' )
                sys.exit( 2 )
