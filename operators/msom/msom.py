...
class msom_parameters( pio ):             # hold arguments values here
    ...
    def __init__( self ):
        ...
        self.nepochs = 4             # number of full data set to sample
        ...
        self.decay_function = 'asymptotic'       # asymptotic (default)
                                                 # constant
                                                 # exponential
                                                 # inverse
                                                 # linear
...
class msom( operator ):
    ...
    def writefile( self, QE, TE, weights, actmap ):
        ...
        try:
            ...
            if isinstance( actmap, np.ndarray ):
                np.save( self.p.actmapfile_prefix + '.npy',
                         actmap, allow_pickle=False )
                #actmap.dump( self.p.activation_map_prefix + '.npy' )
            
        except IOError:
            eprint( 'writefile: IOError' )
    ...
    def init_SOM( self, nbands ) :
        ...
        if self.p.decay_function == 'asymptotic':

            # use default decay_function
            eprint( 'using default asymptotic decay function' )
            
            som = MiniSom( self.p.shape[1], self.p.shape[0],
                           nbands, sigma=self.p.sigma,
                           neighborhood_function=self.p.neighborhood_function,
                           activation_distance=self.p.activation_distance,
                           learning_rate=self.p.rate,
                           random_seed=self.p.seed )

        else:
            if self.p.decay_function == 'constant':
                eprint( 'msom: using constant decay function' )
                decay_function = const_decay
                
            elif self.p.decay_function == 'exponential':
                eprint( 'msom: using exponential decay function' )
                decay_function = exp_decay
                 
            elif self.p.decay_function == 'inverse':
                eprint( 'msom: using inverse decay function' )
                decay_function = inv_decay
                
            elif self.p.decay_function == 'linear':
                eprint( 'msom: using linear decay function' )
                decay_function = linear_decay
                
            elif self.p.decay_function == 'power':
                eprint( 'msom: using power decay function' )
    ...
    def init_weights( self, som, pixels ):
        ...
        if self.p.init_weights == 'random':           
            eprint( 'msom: random initializing neuron weights' )
            som.random_weights_init( pixels )
            
        elif self.p.init_weights == 'pca':           
            eprint( 'msom: pca initializing neuron weights' )
            som.pca_weights_init( pixels )
            
        elif self.p.init_weights == 'custom':
            eprint( 'msom: custom initializing neuron weights' )

            # read custom weight file
            wfile = open( self.p.custom_initfile, 'r' ) 
            ...
            if not found:
                eprint( 'msom: could not find NEURONS flag' )
                sys.exit(2)
    ...
    def epoch_train( self, som, data, nepochs, rorder, show_progress ):
        ...
        for epoch in range( nepochs ):
            ...
            if self.p.calc_epoch_errors and epoch < nepochs-1:
    ...
    def run( self ):                      # override superclass run      
        ...
        self.p.print_params( sys.stderr ) # report parameters
        self.print_versions()
        ...
        QE, TE = self.epoch_train( som, pixels,
                                   self.p.nepochs,
                                   self.p.rorder,
    ...
    def read_params_from_panel( self ):  # scan panel parameters
        ...
        self.p.rate = float( self.t_rate.GetValue().strip() )

        shape_str = self.t_shape.GetValue().strip()
        self.p.shape = self.str2tuple( shape_str )
        ...
        self.p.sigma = float( self.t_sigma.GetValue().strip() )
        self.p.nepochs = int( self.t_nepochs.GetValue().strip() )
        self.p.rate = float( self.t_rate.GetValue().strip() )

        # neighborhood function
        if self.r_gaussian.GetValue():
            self.p.neighborhood_function = 'gaussian'
            
        if self.r_mexican_hat.GetValue():
            self.p.neighborhood_function = 'mexican_hat'
            
        if self.r_bubble.GetValue():
            self.p.neighborhood_function = 'bubble'
            
        if self.r_triangle.GetValue():
            self.p.neighborhood_function = 'triangle'

        # init weights
        if self.r_random.GetValue():
            self.p.init_weights = 'random'
            
        if self.r_pca.GetValue():
            self.p.init_weights = 'pca'

        # topology
        if self.r_rectangular.GetValue():
            self.p.topology = 'rectangular'
            
        if self.r_hexagonal.GetValue():
            self.p.topology = 'hexagonal'

        # activation distance
        if self.r_euclidean.GetValue():
            self.p.activation_distance = 'euclidean'
            
        if self.r_cosine.GetValue():
            self.p.activation_distance = 'cosine'
            
        if self.r_manhattan.GetValue():
            self.p.activation_distance = 'manhattan'
            
        if self.r_chebyshev.GetValue():
            self.p.activation_distance = 'chebyshev'

        # check boxes
        if self.c_activation_map.GetValue():
            self.p.activation_map = True
        else:
            self.p.activation_map = False

        if self.c_calc_epoch_errors.GetValue():
            self.p.calc_epoch_errors= True
        else:
            self.p.calc_epoch_errors = False

        # decay function
        if self.r_asymptotic.GetValue():
            self.p.decay_function = 'asymptotic'
            
        if self.r_constant.GetValue():
            self.p.decay_function = 'constant'
            
        if self.r_exponential.GetValue():
            self.p.decay_function = 'exponential'
            
        if self.r_inverse.GetValue():
            self.p.decay_function = 'inverse'
            
        if self.r_linear.GetValue():
            self.p.decay_function = 'linear'
            
        if self.r_power.GetValue():
            self.p.decay_function = 'power'

        # image output type
        if self.r_label.GetValue():
            self.p.output_type = 'labels'
    ...
    def write_params_to_panel( self ):   # write parameters to panel
        ...
        self.t_shape.SetValue( str(self.p.shape) )
        self.t_sigma.SetValue( str(self.p.sigma) )
        self.t_nepochs.SetValue( str(self.p.nepochs) )
        self.t_rate.SetValue( str(self.p.rate) )

        # neighborhood function
        if self.p.neighborhood_function == 'gaussian':
            self.r_gaussian.SetValue( True )
            
        if self.p.neighborhood_function == 'mexican_hat':
            self.r_mexican_hat.SetValue( True )
            
        if self.p.neighborhood_function == 'bubble':
            self.r_bubble.SetValue( True )
            
        if self.p.neighborhood_function == 'triangle':
        ...
        if self.p.init_weights == 'pca':
            self.r_pca.SetValue( True )

        # topology 
        if self.p.topology == 'rectangular':
            self.r_rectangular.SetValue( True )
            
        if self.p.neighborhood_function == 'hexaganol':
            self.r_hexoganol.SetValue( True )

        # activation distance
        if self.p.activation_distance == 'euclidean':
            self.r_euclidean.SetValue( True )
            
        if self.p.activation_distance == 'cosine':
            self.r_cosine.SetValue( True )
            
        if self.p.activation_distance == 'manhattan':
            self.r_manhattan.SetValue( True )
            
        if self.p.activation_distance == 'chebysev':
        ...
        if self.p.decay_function == 'expontial':          
            self.r_exponential.SetValue( True )
            
        if self.p.decay_function == 'inverse':           
            self.r_inverse.SetValue( True )
            
        if self.p.decay_function == 'linear':
            self.r_linear.SetValue( True )
            
        if self.p.decay_function == 'power':
        ...
        if self.p.calc_epoch_errors == True:
    ...
    def parms_panel( self ):
        ...
        p_parms = wx.Panel( self.p_client, -1, style=wx.SUNKEN_BORDER )
    ...
    def usage( self ):
        ...
        eprint( 'NOTE: this operator has 20 parameters that need to be' )
        eprint( '      itemized for command line execution.' )
        eprint( '      you can use a params file or edit parameters in this script' )
    ...
    def set_params( self, argv ):
        ...
        for opt, arg in opts:
            ...
            if opt in ('-p', '--params'):
                ...
                if not os.path.isfile( arg ):
                    eprint( 'msom: cannot find parameter file:', arg,
                            ' ...exiting' )
                    sys.exit( 2 )
        ...
        if params != None:
            ...
            if not ok:
                eprint( 'msom: set_params: bad params file read' )
                sys.exit( 2 )
