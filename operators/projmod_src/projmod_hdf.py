...
#  18    0.931-0.941                        1km   Atmospheric Water Vapor
...
class projmod_hdf():
    ...
    def open_file( self, filepath, mode='READ' ):   # open the hdf file
        ...
        if mode == 'CREATE':
            ...
            eprint( 'projmod_hdf: open_file: unknown mode:', mode,
                    ' returning' )
            return
    ...
    def get_attr( self, name ):     # get single attribute by name
        ...
        if self.hdf_sd == None:
            eprint( 'projmod: get_atter: hdf file not open' )
            return
        ...
        try:
            ...
            return( 'projmod: getattr: attribute', name, 'unavailable' )
        
    def read_attributes( self ):    # read file attributes
        
        if self.hdf_sd == None:
            eprint( 'projmod_hdf: read_attributes: file not open' )
            return
        
        try:
            self.height,self.width = self.get_attr( 'IMAGE_SIZE_LS' )
            self.pixelsize = self.get_attr( 'PIXEL_SIZE' )
            self.projection = self.get_attr( 'PROJ_TYPE' )

            # determine resolution type by probing
            try:
                hdf = self.hdf_sd.select( 'EV_250_RefSB_b0' )
                self.atype = '250m'
            except:
                pass
            
            try:
                hdf = self.hdf_sd.select( 'EV_500_RefSB_b0' )
                self.atype = '500m'
            except:
                pass
            
            try:
                hdf = self.hdf_sd.select( 'EV_1KM_RefSB_b0' )
                self.atype = '1000m'
            except:
                pass

        except:
            eprint( 'projmod_hdf: read_attributes: cannot determine modis resolution' )
    ...
    def readband_1km( self, band ):
        
        if band in self.EV_250_RefSB:  # process according to band grouping
            
            bufmap = {'1': 0, '2': 1 }
            tagmap = {'1': '0.620-0.670', '2':'0.841-0.876' }

            suffix = '_b' + str(bufmap[band])
            data = self.getband( 'EV_250_Aggr1km_RefSB'+suffix, 
                                  bufmap[band] )
            ...
            suffix = '_b' + str(bufmap[band])
            data = self.getband( 'EV_500_Aggr1km_RefSB'+suffix, 
                                  bufmap[band] )
            return data, tagmap[ band ]
            ...
            suffix = '_b' + str(bufmap[band])
            data = self.getband( 'EV_1KM_RefSB'+suffix, 
                                  bufmap[band] )
            return data, tagmap[ band ]
            ...
            suffix = '_b' + str(bufmap[band])
            data = self.getband('EV_1KM_Emissive'+suffix, bufmap[band] )
            return data, tagmap[ band ]
            ...
            eprint( 'projmod_hdf: readband_1km:  band=', band,
                    'not recognized'  )
            return None,None
    ...
    def readband_500m( self, band ):
        ...
        if band in self.EV_250_RefSB:  # process according to band grouping
            ...
            suffix = '_b' + str(bufmap[band])
            data = self.getband( 'EV_250_Aggr500_RefSB'+suffix, 
                                  bufmap[band] )
            return data, tagmap[ band ]
            ...
            suffix = '_b' + str(bufmap[band])
            data = self.getband( 'EV_500_RefSB'+suffix, bufmap[band] )
            return data, tagmap[ band ]

        else:
            eprint( 'projmod: readband_500m band=', band, 'not recognized'  )
            return None,None
    ...
    def readband_250m( self, band ):
        ...
        if band in self.EV_250_RefSB:  # process according to band grouping
            ...
            suffix = '_b' + str( bufmap[band])
            data = self.getband( 'EV_250_RefSB'+suffix, bufmap[band] )
            return data, tagmap[ band ]
        
        else:
            eprint( 'projmod: readband_250m: band=', band, 'not recognized' )
            return None,None
    ...
    def readband( self, band, swconv='radiance' ):        
        ...
        if band in self.EV_1KM_Emissive:      # overide for emmissive bands
            ...
            if swconv == 'radiance':
                self.conv_scale = 'radiance_scales'
                self.conv_offsets = 'radiance_offsets'
                #eprint( radiance conversion on shortwave band=', band )
                
            elif swconv == 'reflectance':
                self.conv_scale = 'reflectance_scales'
                self.conv_offsets = 'reflectance_offsets'
                #eprint 'reflectance conversion on shortwave band', band )
                
            else:
                eprint( 'projmod_src:read_band: unknown conversion type:',
                        swconv )
                return None, None
        ...
        # TODO: convert radiances to kelvin

    # generate lat,lon buffers
    def make_latlon( self, tl_lat, tl_long, br_lat, br_long ):
        
        nav = np.empty( (self.height,self.width,2),
                        dtype=np.float32 )

        # TODO: make robust wrt to southern, eastern coordinates

        # calculate latitude values
        delta = abs(tl_lat-br_lat)/self.height
        offset = delta/2.0

        for i in range( 0, self.height ):
            lat = tl_lat - i*delta - offset
            nav[i,:,0] = lat

        # calculate longitude values
        delta = abs(tl_long-br_long)/self.width
        offset = delta/2.0

        for i in range( 0, self.width ):
            long = tl_long + i*delta + offset
            nav[:,i,1] = long
